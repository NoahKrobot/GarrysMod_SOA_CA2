...
var debug$2 = srcExports("electron-squirrel-startup");
...
var check = function() {
  if (process.platform === "win32") {
    ...
    debug$2("processing squirrel command `%s`", cmd);
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (isHttp2) {
      ...
      if (config.transport) {
        ...
        transport = isHttpsRequest ? https$2 : require$$3$2;
      } else {
...
const agent = new https$2.Agent({
  rejectUnauthorized: false
  // <- skip TLS verification
});
require$$3$1.ipcMain.on("fetch-data", async (event, args) => {
  try {
...
require$$3$1.ipcMain.on("login", async (event, { username, password }) => {
  let success = false;
  ...
  try {
    ...
    if (userToBeFound) {
      ...
      require$$3$1.ipcMain.emit("routing", event, { success: true, route: "WORKSHOP" });
    } else {
...
require$$3$1.ipcMain.on("routing", async (event, { success, route }) => {
  if (success && route) {
    if (route == "WORKSHOP") {
      console.log("Routing to workshop page...");
      mainWindow.loadFile(
        path$2.join(__dirname, `../../components/html/workshop.html`)
      );
    }
  }
});
